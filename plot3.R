...
plot(mydat2$DateTime, mydat2$Sub_metering_1, type = "n", 
     xlab = "", ylab = "Energy sub metering")                          ## create the plot in the screen device, display only the axes and titles 
lines(mydat2$DateTime, mydat2$Sub_metering_1, col = "black")           ## add the lines to the plot
lines(mydat2$DateTime, mydat2$Sub_metering_2, col = "red")             ## add the lines to the plot
lines(mydat2$DateTime, mydat2$Sub_metering_3, col = "blue")            ## add the lines to the plot
legend("topright", pch = 27, col = c("black", "blue", "red"), 
       legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"),
       lty = 1)                                                        ## add legend to the plot
dev.copy(png, file = "plot3.png")                                      ## copy the plot as a png file in the cwd  
